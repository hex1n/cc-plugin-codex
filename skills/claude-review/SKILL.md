...
Invoke the installed `claude_review_changes` MCP tool with the target Git root. The review is read-only and includes staged, unstaged, and untracked changes. Use `review_profile=quick|standard|deep`; only an explicitly selected deep profile requests Opus. Forward explicitly requested model (including `fable` or a full ID), effort, base, turn, budget, timeout, and background controls. Never chain, retry, upgrade, or add a fallback automatically. Preserve findings, coverage, uncertainty, requested/effective models, token count, cost, turns, duration, and session metadata. For a background review, obtain final usage with `claude_job_result`. If MCP is unavailable, resolve `<PLUGIN_ROOT>` and use `node "<PLUGIN_ROOT>/scripts/claude-companion.mjs" review ...` as the compatibility fallback. This Skill reviews changes only; route an existing plan document to `claude-plan-review`.
