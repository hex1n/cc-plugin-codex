...
export function runCommand(command, args, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", async (code, signal) => { if (timer) clearTimeout(timer); try { await termination; resolve({ code, signal, stdout, stderr, pid: child.pid, timedOut }); } catch (error) { reject(error); } });
