...
export async function createJob({ cwd, profile, resumeSessionId = null, promptMeta = null, write = false, model = null, effort = null, ownerSessionId = codexSessionId(), purpose = "user", namespace = null, disclosure = null, taskProfile = null, reviewProfile = null, maxTurns = null, finalizeAtTurn = null, maxBudgetUsd = null, metadata = {} }) {
  ...
  const parent = metadata.parentJobId
    ? await readJob(cwd, metadata.parentJobId)
    : resumeSessionId ? (await listJobs(cwd)).find(job => job.sessionId === resumeSessionId && ["completed", "failed", "checkpointed", "resuming"].includes(job.status)) : null;
  if (metadata.parentJobId && (!resumeSessionId || parent.sessionId !== resumeSessionId || parent.status !== "resuming")) throw new Error("Explicit task resume parent does not match the claimed session");
