export function renderResult(result, options = {}) {
  ...
  if (json) return JSON.stringify({ ok: true, usage_summary: usageSummary(result, totalTokens), total_tokens: totalTokens, total_cost_usd: result.totalCostUsd ?? null, num_turns: result.numTurns ?? null, duration_ms: result.durationMs ?? null, duration_api_ms: result.durationApiMs ?? null, review_profile: options["review-profile"] ?? null, budget: options["review-profile"] ? { max_turns: options["max-turns"], finalize_at_turn: options["finalize-at-turn"], max_budget_usd: options["max-budget-usd"], timeout_ms: options["timeout-ms"] } : null, result: result.text, structured_output: result.structuredOutput ?? null, session_id: result.sessionId, resume_hint: hint, disclosure: options.disclosure ?? null, usage: result.usage ?? null, model_usage: result.modelUsage ?? null }, null, 2);
...
function jobValue(job) { const start = Date.parse(job.startedAt ?? job.createdAt), end = Date.parse(job.finishedAt ?? new Date().toISOString()); return { id: job.id, record_version: job.recordVersion ?? 1, metadata_completeness: job.metadataCompleteness ?? "legacy-partial", purpose: job.purpose ?? "user", namespace: job.namespace ?? null, workspace: job.cwd ?? null, disclosure: job.disclosure ?? null, status: job.status, phase: job.phase ?? null, progress: job.progressMessage ?? null, duration_ms: Number.isFinite(start) && Number.isFinite(end) ? Math.max(0, end - start) : null, pid: job.pid, profile: job.profile, review_profile: job.reviewProfile ?? null, budget: job.reviewProfile ? { max_turns: job.maxTurns ?? null, finalize_at_turn: job.finalizeAtTurn ?? null, max_budget_usd: job.maxBudgetUsd ?? null, timeout_ms: job.timeoutMs ?? null } : null, write: Boolean(job.write), model: job.model ?? null, created_at: job.createdAt, finished_at: job.finishedAt ?? null, session_id: job.sessionId ?? null, resume_hint: job.sessionId ? `claude --resume ${job.sessionId}` : null, exit_code: job.exitCode ?? null, signal: job.signal ?? null, error_kind: job.errorKind ?? null, upstream_error_subtype: job.upstreamErrorSubtype ?? null, suggested_action: job.suggestedAction ?? null, error: job.error ?? null, cancellation: job.cancellationMode ?? null, prompt_name: job.promptName ?? null, prompt_version: job.promptVersion ?? null, prompt_hash: job.promptHash ?? null }; }
