export function renderResult(result, options = {}) {
  ...
  if (json) return JSON.stringify({ ok: true, operation: options.operation ?? null, review_kind: options["review-kind"] ?? null, subject_kind: options["subject-kind"] ?? null, subject_label: options["subject-label"] ?? null, subject_fingerprint: options["subject-fingerprint"] ?? null, usage_summary: usageSummary(result, totalTokens), total_tokens: totalTokens, total_cost_usd: result.totalCostUsd ?? null, cumulative_chain_cost_usd: options["cumulative-chain-cost-usd"] ?? null, parent_job_id: options["parent-job-id"] ?? null, num_turns: result.numTurns ?? null, duration_ms: result.durationMs ?? null, duration_api_ms: result.durationApiMs ?? null, task_profile: options["task-profile"] ?? null, review_profile: options["review-profile"] ?? null, requested_model: options.model ?? null, effective_models: result.effectiveModels ?? null, effort: options.effort ?? null, budget: profileBudget(options), evidence_lease: renderEvidenceLease(result.evidenceLease), evidence_lease_exhausted: result.evidenceLeaseExhausted ?? false, cost_budget_exhausted: result.costBudgetExhausted ?? false, turn_limit_reached: result.turnLimitReached ?? false, result: result.text, structured_output: result.structuredOutput ?? null, session_id: result.sessionId, resume_hint: hint, disclosure: options.disclosure ?? null, usage: result.usage ?? null, model_usage: result.modelUsage ?? null }, null, 2);
...
function jobValue(job) { const start = Date.parse(job.startedAt ?? job.createdAt), end = Date.parse(job.finishedAt ?? new Date().toISOString()); return { id: job.id, record_version: job.recordVersion ?? 1, metadata_completeness: job.metadataCompleteness ?? "legacy-partial", purpose: job.purpose ?? "user", namespace: job.namespace ?? null, workspace: job.cwd ?? null, disclosure: job.disclosure ?? null, operation: job.operation ?? null, review_kind: job.reviewKind ?? null, subject_kind: job.subjectKind ?? null, subject_label: job.subjectLabel ?? null, subject_fingerprint: job.subjectFingerprint ?? null, transport: job.transport ?? null, capability: job.capability ?? null, evidence_lease: renderEvidenceLease(job.evidenceLease), evidence_lease_exhausted: job.evidenceLeaseExhausted ?? false, task_execution_lease: renderEvidenceLease(job.taskExecutionLease), task_checkpoint: job.taskCheckpoint ?? null, task_completion: job.taskCompletion ?? null, checkpoint_reason: job.checkpointReason ?? null, resume_eligible: job.resumeEligible ?? false, resumed_by_job_id: job.resumedByJobId ?? null, checkpointed_at: job.checkpointedAt ?? null, cost_budget_exhausted: job.costBudgetExhausted ?? false, turn_limit_reached: job.turnLimitReached ?? false, artifact_status: job.artifactStatus ?? null, patch_hash: job.patchHash ?? null, patch_bytes: job.patchBytes ?? null, changed_paths: job.changedPaths ?? null, workspace_backend: job.workspaceBackend ?? null, sandbox_required: job.sandboxRequired ?? false, sandbox_verified: job.sandboxVerified ?? false, sandbox_policy_hash: job.sandboxPolicyHash ?? null, claude_executable_sha256: job.claudeExecutableSha256 ?? null, cleanup_pending: job.cleanupPending ?? false, recovery_required: job.recoveryRequired ?? false, context_drift_detected: job.contextDriftDetected ?? false, context_drift_path_count: job.contextDriftPathCount ?? 0, context_drift_confirmed: job.contextDriftConfirmed ?? false, status: job.status, phase: job.phase ?? null, progress: job.progressMessage ?? null, duration_ms: job.durationMs ?? (Number.isFinite(start) && Number.isFinite(end) ? Math.max(0, end - start) : null), duration_api_ms: job.durationApiMs ?? null, pid: job.pid, profile: job.profile, task_profile: job.taskProfile ?? null, review_profile: job.reviewProfile ?? null, budget: job.taskProfile || job.reviewProfile ? { max_turns: job.maxTurns ?? null, finalize_at_turn: job.finalizeAtTurn ?? null, soft_budget_usd: job.maxBudgetUsd ?? null, max_budget_usd: job.maxBudgetUsd ?? null, timeout_ms: job.timeoutMs ?? null } : null, write: Boolean(job.write), model: job.model ?? null, requested_model: job.requestedModel ?? job.model ?? null, effective_models: job.effectiveModels ?? null, effort: job.effort ?? null, created_at: job.createdAt, finished_at: job.finishedAt ?? null, session_id: job.sessionId ?? null, resume_hint: job.sessionId ? `claude --resume ${job.sessionId}` : null, parent_job_id: job.parentJobId ?? null, cumulative_chain_cost_usd: job.cumulativeChainCostUsd ?? null, exit_code: job.exitCode ?? null, signal: job.signal ?? null, error_kind: job.errorKind ?? null, upstream_error_subtype: job.upstreamErrorSubtype ?? null, suggested_action: job.suggestedAction ?? null, error: job.error ?? null, total_cost_usd: job.totalCostUsd ?? null, num_turns: job.numTurns ?? null, usage: job.usage ?? null, model_usage: job.modelUsage ?? null, cancellation: job.cancellationMode ?? null, prompt_name: job.promptName ?? null, prompt_version: job.promptVersion ?? null, prompt_hash: job.promptHash ?? null }; }
...
function renderEvidenceLease(value) {
...
function errorDetails(error) {
  return {
    ...
    review_kind: error.reviewKind ?? null,
    subject_kind: error.subjectKind ?? null,
    subject_label: error.subjectLabel ?? null,
    subject_fingerprint: error.subjectFingerprint ?? null,
    ...
    evidence_lease_exhausted: error.evidenceLeaseExhausted ?? false,
    cost_budget_exhausted: error.costBudgetExhausted ?? error.errorKind === "max_budget",
    turn_limit_reached: error.turnLimitReached ?? error.errorKind === "max_turns"
  };
}
