...
async function fixture() {
  ...
  await writeFile(fakeClaude, `#!/usr/bin/env node\nimport {writeFileSync} from "node:fs";\nif(process.argv[2]==="--version"){console.log("9.9.9");process.exit(0)} if(process.argv[2]==="auth"){console.log(JSON.stringify({loggedIn:true,authMethod:"oauth"}));process.exit(0)} writeFileSync(process.env.FAKE_CALLED,"yes"); if(process.env.FAKE_FAIL==="1"){console.error("review unavailable");process.exit(1)} console.log(JSON.stringify({type:"result",is_error:false,result:"",structured_output:JSON.parse(process.env.FAKE_VERDICT),session_id:"gate-session"}));\n`);
