...
async function fixture() {
  ...
  await mkdir(cwd);
  await writeFile(fake, `#!/usr/bin/env node\nimport {writeFileSync} from "node:fs"; const args=process.argv.slice(2);let prompt="";for await(const chunk of process.stdin)prompt+=chunk;writeFileSync(process.env.FAKE_ARGS,JSON.stringify({args,prompt})); console.log(JSON.stringify({type:"result",is_error:false,result:"resumed",session_id:"continued-session"}));\n`);
...
test("foreground task forwards a resume session while keeping the prompt on stdin", async () => {
  ...
  const { args, prompt } = JSON.parse(await readFile(fx.invocation, "utf8")), resume = args.indexOf("--resume");
  assert.ok(resume > 0); assert.equal(args[resume + 1], "source-session"); assert.equal(args.indexOf("--"), -1); assert.match(prompt, /<task>\s*continue work[\s\S]*Beginning with turn 6/);
});
...
test("background task persists and forwards its resume session", async () => {
  ...
  for (let i = 0; i < 100; i += 1) { try { const { args, prompt } = JSON.parse(await readFile(fx.invocation, "utf8")); assert.equal(args[args.indexOf("--resume") + 1], "background-source"); assert.match(prompt, /continue later/); return; } catch { await new Promise(resolveWait => setTimeout(resolveWait, 20)); } }
