...
任务默认使用只读 `standard` profile：Sonnet、medium effort、最多 8 turns、第 6 turn 开始收口、$1.50 软预算和 300 秒超时。只有在确实需要 Claude 修改工作区时，才使用 `/claude-task --write`。常用参数包括 `--task-profile`、`--model`、`--effort`、`--max-turns`、`--finalize-at-turn`、`--max-budget-usd`、`--prompt-file`、`--resume`、`--continue`、`--fresh` 和 `--background`。
