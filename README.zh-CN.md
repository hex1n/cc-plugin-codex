...
Codex 的唯一正常入口是 `.mcp.json` 声明的本地 stdio MCP server。13 个 typed tools 覆盖只读任务、代码/方案/对抗审查、隔离写任务启动、显式 checkpoint resume、显式 apply/discard、显式 ID 的 job 生命周期、有界 job 列表和只读诊断。MCP server 通过稳定的 `#app/*` contract 调用 application service，不通过 shell，也不会 spawn CLI adapter。Prompt 通过 stdin 传给 Claude，不进入 argv。
...
推荐的低保真产品模型是对话优先：先选择 Task 或专用 Review 意图，再跟踪对应 Job，最后按状态显式 Resume、Cancel、Apply 或 Discard。Jobs、Doctor 和 Transfer 是辅助入口，不是插件自有的独立后台。

![cc-plugin-codex 全仓 Balsamiq 风格线框原型](docs/prototypes/cc-plugin-codex-low-fidelity.svg)

故事板将九个 Skills 与 13 个 typed MCP tools 映射到 Intent → Job → Decision 主线，并标出 isolated-write 可信边界。详细说明见
[全仓原型说明](docs/prototypes/cc-plugin-codex-low-fidelity.md)。
...
本地 marketplace checkout 可使用等价的官方管理命令：

```sh
codex plugin add cc-plugin-codex@personal --json
```

重新安装会刷新版本化本机缓存；完成后仍需重启 Codex，使 Skills 和 hooks 重新加载。
...
`context=summary|diff|full` 声明发送的上下文粒度，不会暗中改写 prompt。旧兼容只读续接只允许二选一：`resume_session_id` 或 `continue_session`，默认仍是新任务。启用 Task Execution Lease 后，只读与隔离写 checkpoint 只能通过 `claude_task_resume(job_id)` 显式续跑；隔离写会复用并重新验证原 sandbox。结果和后台状态会返回简短披露摘要。`finalize_at_turn` 用于提示 Claude 在硬 turn 上限前开始停止扩展调查并收口。
