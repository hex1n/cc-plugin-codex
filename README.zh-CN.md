...
只要 Claude CLI 提供相应字段，完成结果和结构化预算失败都会输出 token 使用量、各模型使用量、总成本、turn 数、API 耗时和总耗时。JSON 结果额外提供跨模型的 `total_tokens` 汇总字段；旧版 CLI 缺少的数据会返回 `null`。
...
审查 profile 会限制 turn、成本和墙钟时间，但不会自动串行调用多个模型。每次 review 必须报告已检查/跳过文件、不确定性、预算是否耗尽，以及需要时的定向 follow-up profile。小范围扫描使用 `quick`，日常审查使用 `standard`，安全、并发、迁移和核心状态机使用 `deep`。CLI 预算参数可以覆盖本次调用的 profile 默认值。

Claude 的 `--max-turns` 限制 agentic turns，而 CLI 返回的 `num_turns` 使用更宽的 usage 计数口径，因此数值可能更大。插件把美元预算和墙钟超时作为硬执行上限，同时保留两种 turn 数据用于审计。
...
终态 phase 与状态保持一致：`done`、`failed`、`cancelled`、`timed_out`。Claude 的 `error_max_turns` 等原始错误 subtype 会被保留并映射为稳定错误类型、恢复建议和可用的 resume session。旧版任务只在读取时归一，并标记为 `legacy-partial`，不会回写历史文件。
