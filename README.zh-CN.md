...
Codex 主入口是 `.mcp.json` 声明的本地 stdio MCP server。它分别暴露只读任务、代码审查、方案审查、隔离写任务启动、显式 apply/discard 和 job 生命周期工具。MCP server 与兼容 CLI 直接复用同一个 application service，不通过 shell，也不会 spawn CLI adapter。Prompt 通过 stdin 传给 Claude，不进入 argv。
...
任务默认使用只读 `standard` profile：Sonnet、medium effort、最多 8 turns、第 6 turn 开始收口、$1.50 软预算和 300 秒超时。只有在确实需要 Claude 修改工作区时，才使用 `/claude-task --write`。写任务会在独立 clone 中运行并停在 `awaiting_apply`，需要另一次明确 apply 才会修改源工作区。常用参数包括 `--task-profile`、`--model`、`--effort`、`--max-turns`、`--finalize-at-turn`、`--max-budget-usd`、`--prompt-file`、`--resume`、`--continue`、`--fresh` 和 `--background`。
...
Claude 启动前会拒绝未知 section、未知字段、无效 JSON、非正数限制，以及超过内置安全上限的 review 配置。写权限不能写入配置文件；每个可写任务仍必须显式传入 `--write`。

Codex 提供 `PLUGIN_DATA` 时，插件任务和配置存放在其中；直接运行脚本时，回退到用户的 Codex 数据目录。任务记录按工作区隔离，并采用原子写入。

只要 Claude CLI 提供相应字段，完成结果和结构化预算失败都会输出请求模型、实际模型、token 使用量、各模型使用量、总成本、turn 数、API 耗时和总耗时。JSON 结果额外提供跨模型的 `total_tokens` 汇总字段；旧版 CLI 缺少的数据会返回 `null`。
...
审查 profile 会限制 turn、软预算和墙钟时间，但不会自动串行调用或 fallback 到其他模型。gate、quick、standard 请求 Sonnet；只有显式 deep review 请求 Opus。每次 review 必须报告已检查/跳过文件、不确定性、预算是否耗尽，以及需要时的定向 follow-up profile。小范围扫描使用 `quick`，日常审查使用 `standard`，安全、并发、迁移和核心状态机使用 `deep`。CLI 预算参数可以覆盖本次调用的 profile 默认值。

Claude 的 `--max-turns` 限制 agentic turns，而 CLI 返回的 `num_turns` 使用更宽的 usage 计数口径，因此数值可能更大。美元预算是 Claude CLI 可能略微超过的软目标；只有墙钟超时由插件强制作为硬执行上限。插件会保留两种 turn 数据和最终成本用于审计。

大型 review 使用有界 manifest，不会把所有变更路径都写进初始 prompt。Claude 可以通过内置只读 diff adapter 按需读取有限大小的 patch，输出上限与仓库规模无关。现有 256 KiB inline 紧急上限保留，不引入 96 KiB 的 review 覆盖限制。

长输入优先使用 `--prompt-file`，避免完整正文进入 shell 命令和审批展示。`--context summary|diff|full` 声明发送的上下文粒度，不会暗中改写 prompt。JSON 结果和后台状态会返回简短披露摘要。设置 `--max-turns` 时可以同时使用 `--finalize-at-turn <n>`，提示 Claude 从指定轮次开始停止扩展调查并收口。
...
- 可选 Stop review gate 默认关闭。使用 `setup --enable-review-gate` 开启，在 Codex 中检查并信任该 hook；使用 `setup --disable-review-gate` 关闭。gate 使用独立预算（默认 4 turns、$0.20、90 秒），并缓存 30 分钟内未变化的 verdict，避免重复 Stop 反复调用模型。
...
不传 ID 的 `status` 返回当前 Codex 会话的最新任务；`--all` 返回工作区完整历史。显式 `--global` 查询所有持久化工作区，默认隐藏 E2E 任务，使用 `--include-test` 才显示；还可以用 `--recent 30m|24h|7d`、`--status` 和 `--purpose` 过滤。不传 ID 的 `result` 和 `cancel` 会选择当前会话中最新的适用任务。会话开始时会校正失效记录；会话结束时会清理过期的已完成记录，但绝不会终止仍在运行的任务。
